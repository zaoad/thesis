...
def cluster_calculation(pid,factornodes,variablenodes,variables_total_adjacent, factors_total_adjacent,variables_send_message,variables_final_send_message,factors_send_message, factors_final_send_message,final_values):
    while (True):
        ...
        #for f in factornodes:
            #print(f,factornodes[f].variables)
        #for v in variablenodes:
            #print(v,variablenodes[v].factors_nodes)
...
if __name__ == '__main__':
    ...
    '''
    cluster,factortocluster,adjacentcluster= make_cluster(total_factor_count,max_fact_in_cluster,adjacentfactor,linker_variables)
    ...
    for i in range(len(cluster)):
        ...
        if(len(adjacentcluster[i])==1):
            p=Process(target=cluster_calculation,args=(i, c_factornodes[i],c_variablenodes[i],c_variables_total_adjacent[i],c_factors_total_adjacent[i],c_variables_send_message[i]
                                     ,c_variables_final_send_message[i],c_factors_send_message[i],c_factors_final_send_message[i],final_values[i],))
    ...
    for i in range(total_cluster):
        ...
        for cvar in inclustercivar[i]:
            ...
            p=Process(target=intermediatestepallfact,args=(factnode,ci_variable,ci_values,factornodes,adjacentfactor,cvar
    ...
    for i in range(total_cluster):
        for cvar in inclustercivar[i]:
            #print('i','cvar',i,cvar,c2_variablenodes[i][cvar])
    ...
    for i in range(len(cluster)):
        ...
        p = Process(target=cluster_calculation, args=(i,c2_factornodes[i],c2_variablenodes[i],c2_variables_total_adjacent[i],c2_factors_total_adjacent[i],c2_variables_send_message[i]
                                     ,c2_variables_final_send_message[i],c2_factors_send_message[i],c2_factors_final_send_message[i],final_values1[i],))
